    ...
    protected synchronized void applyAttack(AttackInfo attack, MapleCharacter player, int maxDamagePerMonster, int attackCount) {
		...
		if (attack.skill == 4211006) {
                ...
                if (damageDealtToMob >= 250000 && attack.skill != 3221001 && attack.skill != 4211006) {
                    ...
                    (player.getClient(),"[AUTOBAN]" + player.getName() + " dealt " + damageDealtToMob + " to monster " + monster.getId() + ".");
                }
                if (damageDealtToMob >= 70000 && attack.skill == 0 && player.getLevel() < 200) {
                    AutobanManager.getInstance().autoban
                    (player.getClient(),"[AUTOBAN]" + player.getName() + " dealt " + damageDealtToMob + " to monster " + monster.getId() + " at level " + player.getLevel() + " with only a basic attack...");
                }
        ...
        for (int i = 0; i < ret.numAttacked; i++) {
            ...
            for (int j = 0; j < ret.numDamage; j++) {
                ...
                if ((damage > chr.calculateWorkingDamageTotal(chr.getTotalWatk())*1.3) && ret.skill == 0 
                    ...
                    FilePrinter.printError(FilePrinter.Damage + chr.getName() + chr.getJob().getId() + ".txt", "Player dealt " + damage + " with max range of " 
                            + chr.calculateWorkingDamageTotal(chr.getTotalWatk()) + " using a basic attack. Player was level " + chr.getLevel() 
                            + ". Player was JobID " + chr.getJob().getId() + "\n");
