...
var mapid = new Array(103000900,103000903,103000906,600000000);
...
function action(mode, type, selection) {
	if (mode == -1 || (mode == 0 && status ==1)) {
		...
		if (mode == 1)
			status++;
		if (status == 0) {
			if (cm.haveItem(itemid[0], 1) || cm.haveItem(itemid[1], 1) || cm.haveItem(itemid[2], 1) || cm.haveItem(itemid[3], 1)) {
				status = 1;
			} else {
