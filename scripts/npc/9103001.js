	...
	if (status == 0) {
	    ...
	    if(selection == 0) {//ENTER THE PQ
			if (!hasParty()) {//NO PARTY
				...
				cm.sendOk("One of your party members has not met the level requirements of 50~120.");
...
function checkPartyLevels(){
    ...
    if (cm.getParty() == null) {
		...
		for (var i = 0; i < party.size() && pass; i++) {
			if ((party.get(i).getLevel() < 50) || (party.get(i).getLevel() > 85) || (party.get(i).getMapid() != cm.getMapId())) {
				pass = false;
			}
		}
    }
