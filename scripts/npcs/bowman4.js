/**
 * Legor: 4th Job - Bowman Instructor (NPC 2081300)
...
if (player.getJob() == 312 || player.getJob() == 322) {
	npc.sayNext("How are your journeys as a master bowman? Please use your strength for the good of the Maple World. Not everyone is as capable and fortunate as you.");
} else if ((player.getJob() == 311 || player.getJob() == 321) && player.getLevel() >= 120 && player.isQuestCompleted(6924)) {
	let selection = npc.askYesNo("You must have trained your whole life for this. Are you prepared for your 4th job advancement?");
	if (selection == 1) {
		if (player.getJob() != 311 && player.getJob() != 321 || player.getLevel() < 120 || !player.isQuestCompleted(6924)) {
			npc.logSuspicious("Tried to select NPC option that was not given");
		}
		advanceJob();
	} else {
