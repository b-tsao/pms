...
function start(mode, type, selection) {
	if (mode == -1) {
		...
		if (status == 0) {
			...
			qm.sendAcceptDecline("Then... Let me liven things up a bit! Abaracadabra~!");
		} else if (status == 3) {
		playerhp = qm.getPlayer().getHp()/2;
			qm.getPlayer().setHp(playerhp);
			qm.getPlayer().updateSingleStat(MapleStat.HP, playerhp);
...
function end(mode, type, selection) {
	if (mode == -1) {
		...
		if (status == 0) {
		if (qm.haveItem(2010007)) {
			qm.sendOk("You need to eat the #bRoger's Apple!#k");
			mode = -1;
		}
		...
		if (qm.getPlayer().getQuestStatus(1021) == 1) {
			qm.forceCompleteQuest(1021);
			qm.gainExp(10);
