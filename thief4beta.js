...
function advanceJob() {
	let selection = npc.askYesNo("You will now become the strongest among thieves. Do you wish to make the job advancement now?");
	if (selection == 1) {
		if (player.getJob() != 411 && player.getJob() != 421 || player.getLevel() < 120) {
...
if ((player.getJob() == 411 || player.getJob() == 421) && player.getLevel() >= 120 && player.isQuestCompleted(6934))
	...
	if (selection == 1) {
		...
		if (player.isQuestCompleted(6934)) {
			advanceJob();
		} 
	else if (selection == 0) {
	...
	else if (player.getJob() == 412 || player.getJob() == 422) {
